...
Repository-owned HDL and class sources use the `.sv` suffix. Package entry
points such as `src/gq/gq_pkg.sv`, each business library's `*_pkg.sv`, and the
matching `tb/*_test_pkg.sv` include their component `.sv` files; those included
files are not separate build-system source units. The third-party UVM
`uvm_macros.svh` include is the only `.svh` dependency referenced by repository
sources.
...
## Use the standard UVM agent and monitor

Every explicitly configured queue creates one active UVM agent with this
component hierarchy:
...
`gq_driver` handles TX submission and the initial RX start. `gq_monitor` owns
the completion observation loop for both Poll and IRQ configurations and
publishes completed descriptors only through its `completion_ap`.
`gq_queue_engine` owns ring state, descriptor lifetime, refill, reset, and
concurrency, but its completion-port handle is protected. The agent binds the
monitor's port exactly once during `connect_phase`; code must not reach into an
engine to connect an analysis subscriber.

A scoreboard connects to the monitor obtained from the environment:
...
Focused tests that instantiate a `gq_queue_engine` without an agent may create
their own `uvm_analysis_port #(gq_desc_base)` and pass it once to
`bind_completion_port()`. Production environments should use the standard
monitor path above. Passive-agent mode is not currently implemented.
...
```bash
./scripts/run_vcs_remote.sh gq_timing_config_test mailbox
./scripts/run_vcs_remote.sh gq_wait_policy_test mailbox
./scripts/run_vcs_remote.sh gq_worker_wakeup_test mailbox
./scripts/run_vcs_remote.sh gq_config_test mailbox
./scripts/run_vcs_remote.sh gq_index_phase_ptr_codec_test mailbox
...
./scripts/run_vcs_remote.sh gq_agent_test mailbox
./scripts/run_vcs_remote.sh gq_async_completion_test mailbox
./scripts/run_vcs_remote.sh gq_refill_test mailbox
./scripts/run_vcs_remote.sh gq_auto_recycle_test mailbox
./scripts/run_vcs_remote.sh gq_refill_batch_test mailbox
