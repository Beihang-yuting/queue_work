// tb/tests/gq_refill_batch_test.sv: UVM 测试 gq_refill_batch_test：验证对应队列组件的定向行为和接口契约。
...
class gq_refill_batch_test extends uvm_test;
    ...
    task run_phase(uvm_phase phase);
        ...
        fork : refill_workers
            begin
                bounded_engine.run_completion_monitor();
                bounded_worker_returned = 1;
            end
            begin
                unlimited_engine.run_completion_monitor();
